<script>
	import { deleteTodo, toggleTodo, editTodo } from '../../stores/TodoStore.js';
	export let todo;

	$: complete = todo.completed ? 'bg-green-three' : 'bg-leaf-one';
</script>
...
<div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
	<div class="flex w-full max-w-lg items-center justify-start">
		<label for={`${todo.id}-checkbox`} class="sr-only">Complete</label>
		<input
			type="checkbox"
			id={`${todo.id}-checkbox`}
		...
		/>
		<label for={`${todo.id}-input`} class="sr-only">Edit</label>
		<input
			type="text"
			id={`${todo.id}-input`}
			value={todo.text}
		...
		/>
		<span
			class="{complete} ml-5 hidden rounded-full px-2 py-0.5 text-sm font-normal text-gray-five md:block"
		>
